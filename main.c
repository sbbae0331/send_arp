...
int main(int argc, char* argv[]) {
  ...
  // step 1: B 가 자신의 MAC (BMAC) 과 IP (BIP) 를 얻는다
	...
	if (0 == ioctl(fd, SIOCGIFADDR, &s))
		for (int i = 0; i < 4; i++) B_IP[i] = s.ifr_addr.sa_data[i+2];
	close(fd);

	uint32_t A_IP_HEX = inet_addr(argv[2]);
	for (int i = 0; i < 4; i++) A_IP[i] = *((uint8_t *)&A_IP_HEX + i); // HEX to array
	
	uint32_t AP_IP_HEX = inet_addr(argv[3]);
	for (int i = 0; i < 4; i++) AP_IP[i] = *((uint8_t *)&AP_IP_HEX + i); // HEX to array
